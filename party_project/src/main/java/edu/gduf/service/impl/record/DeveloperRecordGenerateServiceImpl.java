 ...
 **/
@Service
public class DeveloperRecordGenerateServiceImpl implements FileGenerateService {

    private StudentDao studentDao;
    /**
     * 落款日期以及确定为发展对象的日期
     */
    private String identifyingDeveloperTime = null;
    ...
    public ResultInfo fileGenerate(List<String> list) {
        ...
        return ResultInfo.successInfo("成功生成"+branch+"确定发展对象备案表",filepath);
    }

    private String generate(List<List<String>> information) throws IOException {
        //加载模板
        InputStream in = Resources.getResourceAsStream("model/record/developer_record_table.doc");
        Document doc = new Document(in);

        //替换指定文本
        doc.replace("branch", branch, true, true);
        doc.replace("time", identifyingDeveloperTime, true, true);
        ...
        for (List<String> data : information) {
            ...
            tableRow.getCells().get(9).addParagraph().setText(data.get(6));
            tableRow.getCells().get(10).addParagraph().setText(data.get(7));
            tableRow.getCells().get(11).addParagraph().setText(identifyingDeveloperTime);
        }
        ...
        doc.saveToStream(out, FileFormat.Doc);
    ...
    public List<List<String>> getInformation(List<String> list) {
        ...
        for (String s : list) {
            ...
            if (branch == null && identifyingDeveloperTime == null) {
                branch = student.getPartyBranch();
                String[] split = student.getDeveloper().getIdentifyingDeveloper().split("-");
                identifyingDeveloperTime = split[0]+"年"+split[1]+"月"+split[2]+"日";
            }
